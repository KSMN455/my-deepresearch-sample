...
def _research_agent(brief: InstructionBrief, trace: TraceLog) -> str:
    ...
    max_tokens = int(os.getenv("RESEARCH_MAX_OUTPUT_TOKENS", "40000"))
    ...
    # ステータスが incomplete の場合はトレースに詳細を記録しておく。
    try:
        ...
        if status == "incomplete" and incomplete:
            try:
                content = json.dumps(
                    getattr(incomplete, "model_dump", lambda: incomplete)(),
                    ensure_ascii=False,
                    indent=2,
                )
            except Exception:
                # 文字列化に失敗しても致命的ではない
                content = str(incomplete)
            _log_trace(
                trace,
                stage,
                kind="info",
